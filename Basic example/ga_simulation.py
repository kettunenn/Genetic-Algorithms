...
def create_population(individuals, genome):
...
def roulette_selection(population,pop_fitness, individuals):
...
def crossover(population, p_crossover,genome,p_mutation):
...
def simulation(genome, individuals, p_crossover, p_mutation, max_iterations):
    ...
    pop = create_population(individuals,genome)
    ...
    while True:
        ...
        parents_AB = roulette_selection(pop, pop_fitness, individuals)
        offspring_AB = crossover(parents_AB,p_crossover,genome,p_mutation)                        
        ...
        if 50 in pop_fitness or t == max_iterations:
            return fitness_growth, t
