...
def create_individual_2(LOWER_BOUNDS, UPPER_BOUNDS, state, N_HOURS=24):
    ...
    bool, B = sys.battery_sim(D,Gb,g, state, Bmax=100, N_HOURS=24)

    Gb = np.random.uniform(0, Gbmax, size=N_HOURS)

    g = np.random.uniform(0, gmax, size=N_HOURS)

    return np.concatenate([D, Gl, Gb, g]), B
